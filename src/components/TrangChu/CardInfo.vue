            ...
            <div
                ...
                <div class="hidden py-4 xl:block xl:px-6">
                    <h3 class="text-xl font-semibold text-gray-800 dark:text-white">{{
                        this.infoData?.FullName?.[this.getLang()] || ""
                        }}</h3>
                    <h5 class="py-2 text-gray-700 dark:text-gray-300">{{ this.infoData.Job[this.getLang()] }} | {{
                        this.trans?.home_body?.favorite?.[this.getLang()] || "" }} {{
                            this.infoData?.Favorite?.[this.getLang()] || ""
                        }}</h5>
                    <div class="mt-4 mb-2 space-y-4 text-black dark:text-gray-100">
                        <div class="flex items-center text-gray-700 dark:text-gray-200">
                            ...
                            <p class="flex items-center px-2 space-x-1">
                                <span>{{ this.infoData?.Workplace?.[this.getLang()] || "" }}</span>
                            </p>
                        </div>
                        <div class="flex items-center text-gray-700 dark:text-gray-200">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor"
                            ...
                            <p class="px-2">{{ this.infoData?.Address?.[this.getLang()] || "" }} <span
...
export default {
    ...
    async mounted() {
        ...
        this.isLoading = !this.isLoading; // Ẩn loading spinner
    }
}
</script>