<template>
    ...
    <div class="xl:block hidden">
        <!--Loading Spinner-->
        <div class="flex justify-center items-center h-4/6" v-if="this.isLoading">
        ...
        <div class="z-10 hover:z-50 mb-8 xl:mb-0 transition-all duration-200 ease-out scale-100 xl:hover:scale-[1.15]"
            style="perspective: 600px;" v-if="this.infoData !== null && Object.keys(this.infoData).length > 0 && !this.isLoading">
            <div
                ...
                <div class="xl:block hidden xl:px-6 py-4">
                    ...
                    <h5 class="py-2 text-gray-700 dark:text-gray-300">{{ this.infoData.Job }} | Sở thích {{ this.infoData.Favorite
                        }}</h5>
                    <div class="space-y-4 mt-4 mb-2 text-black dark:text-gray-100">
                        <div class="flex items-center text-gray-700 dark:text-gray-200">
                            ...
                            <p class="flex items-center space-x-1 px-2">
                                <span>{{ this.infoData.Workplace }}</span>
                            </p>
                        </div>
                        <div class="flex items-center text-gray-700 dark:text-gray-200">
                            <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor"
                            ...
                            <p class="px-2">{{ this.infoData.Address }} <span class="inline-flex absolute ml-1 pt-px">
                        ...
                        <div class="flex items-center gap-2.5 text-gray-700 dark:text-gray-200 ml-[0.20rem]">
                            ...
                            <a target="_blank" :href="this.infoData.Socials.X.Url" class="flex items-center text-sm hover:underline">
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor"
...
export default {
    ...
    async mounted() {
        ...
        this.isLoading = true; // Hiển thị loading spinner
        this.infoData = await this.getCaNhan(); // Dữ liệu thông tin cá nhân dưới dạng JSON
        this.isLoading = false; // Ẩn loading spinner
    },
}
</script>